 ...
 * {@link #updateReadyToSave(boolean) } like updateReadyToSave(oldValue !=
 * newValue)</li>
 * <li>for immediate save, call {@link #save() } in each setter after data
 * change ({@link #readyToSave } must be set) </li>
 * </ul>
 * <p>
 * For use this class, you need add those libraries: jackson-core
...
//  }
  ...
  * @return object with loaded data for given class or new instance, if file not
  * exist
  * @throws java.io.IOException
  * @throws java.lang.InstantiationException
  * @throws java.lang.IllegalAccessException
 ...
 public static <T extends JSONSaver> T load(Class<T> clazz,
  ...
  if (log.isTraceEnabled()) {
   ...
   if (log.isTraceEnabled()) {
    log.trace("Clazz = {}, File = {}", clazz, Arrays.toString(files));
   }
  }
  T stor;
  loading = true;
  ...
  if (exists) {
   ...
   JsonNode source = json.readTree(file[0]);
   for (int i = 1; i < file.length; i++) {
    try {
     JsonNode update = json.readTree(file[i]);
     source = JsonUtil.merge(source, update);
    } catch (IOException e) {
     log.error(String.format("Error on load data from %s", file[i].getAbsoluteFile()), e);
     throw e;
    }
   }
   stor = json.treeToValue(source, clazz);
  } else {
   log.info("File [{}] not found, create new instance of [{}]", file[0].getAbsolutePath(), clazz.getCanonicalName());
